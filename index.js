...
console.log(
  createSymbolProperty(
    "Це властивість об'єкту з використанням символу",
  ),
...
function createAndCompareTwoSymbols(value) {
  const sym1 = Symbol(value) // Створюємо два символи з однаковим описом value, та повертаємо результат порівняння двох символів
...
function compareSymbolsForKey(symbol1, symbol2) {
  if (symbol1 === symbol2) {
    ...
    return true // Якщо два символи однакові, він поверне true, в іншому випадку - false
  } else {
    return false
  }
}
...
function useSymbolToStringTag() {
  ...
  return myObject[Symbol].toString() // Повертаємо рядкове представлення myObject[Symbol].
}
...
let myObject = {
  ...
  next() {
    ...
    if (this.currentIndex < this.category.length) {
      ...
      return { value: value, done: false } // Повертаємо об'єкт з властивістю value значенням якої буде value,та прапорцем done: false
    } else {
